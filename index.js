  ...
  if (taskQueues[threadID].length === 1) {
    (async () => {
      while (taskQueues[threadID].length > 0) {
        ...
        await sleep(getRandomDelay(2000, 5000)); // 2-5 सेकंड
      }
    })();
  }
}
...
async function loginAndRun() {
  while (!shuttingDown) {
    loginAttempts = 0; // रीसेट loginAttempts हर साइकिल में
    while (!shuttingDown && loginAttempts < MAX_LOGIN_ATTEMPTS) {
      try {
        ...
        await loadLocks();
        loginAttempts = 0; // सफल लॉगिन पर रीसेट
        // शुरू करें initCheckLoop
        setInterval(() => initCheckLoop(api), 30 * 60 * 1000); // 30 मिनट
        // Auto-save AppState
        setInterval(() => {
          try {
        ...
        if (e.message.includes("Error retrieving userID")) {
          error("Session likely invalid. Please generate a new appstate.json.");
          break; // अमान्य सेशन पर लूप से बाहर निकलें
        }
        ...
        if (loginAttempts >= MAX_LOGIN_ATTEMPTS) {
          warn("Max login attempts reached. Waiting before retrying...");
          break;
        }
        const backoff = Math.min(300, (loginAttempts + 1) * 60); // 1-5 मिनट
        info(`Retrying login in ${backoff}s...`);
        await sleep(backoff * 1000);
      }
    }
    if (loginAttempts >= MAX_LOGIN_ATTEMPTS || shuttingDown) {
      warn("Pausing login attempts for 30 minutes...");
      await sleep(30 * 60 * 1000); // 30 मिनट का डिले
    }
  }
}
